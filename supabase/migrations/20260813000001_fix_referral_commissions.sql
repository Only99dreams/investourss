-- Fix: FHA ambassador commissions never reflected in the Referrals dashboard.
--
-- record_ambassador_commission() (Aug-04/08/12) recorded commissions into the
-- commissions/ambassadors/wallets tables but NEVER updated
-- referral_stats.total_earnings — which is the value the Referrals dashboard
-- "Total Earnings" card reads (src/components/dashboard/sections/ReferralsSection.tsx).
-- Result: ambassador payouts were recorded but the UI showed ₦0 forever.
--
-- This migration is idempotent and safe to re-run:
--   * rewrites record_ambassador_commission() (direct 30%/15% on the
--     VAT-exclusive amount + 2% indirect level-2 bonus) so it also maintains
--     referral_stats.total_earnings for both the direct and indirect payout,
--   * backfills referral_stats.total_earnings from the commissions ledger so
--     already-earned commissions show up immediately.

-- ============================================================
-- 1. Rewrite record_ambassador_commission
-- ============================================================
...
-- ============================================================
-- 2. Backfill referral_stats.total_earnings from commissions
-- ============================================================
