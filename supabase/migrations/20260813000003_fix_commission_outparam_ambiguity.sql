-- Fix: record_ambassador_commission fails with 42702 "column reference
-- 'ambassador_id' is ambiguous".
--
-- Root cause: the function is declared `RETURNS TABLE (... ambassador_id uuid,
-- referral_id uuid)`. Those output-parameter names shadow the real table
-- columns `ambassadors.ambassador_id` / `referrals.ambassador_id` in the
-- PL/pgSQL body. When execution reaches
--
--   INSERT INTO referrals (ambassador_id, referred_user_id, status, created_at)
--   VALUES (...) ON CONFLICT (ambassador_id, referred_user_id) DO UPDATE ...
--
-- PostgreSQL cannot tell whether `ambassador_id` is the table column or the
-- PL/pgSQL variable and raises 42702. This existed since the Aug-04 ambassador
-- system but only fires the first time a qualifying commission runs, which is
-- why no commission was ever recorded: the ONE attempt (the subscription paid
-- by ambassador 3ed393f3 on Aug-12) hit this error and was logged to
-- commission_failures.
--
-- Fix: rename the output parameters to out_* so they can never collide with
-- table column names. The function signature (uuid, numeric, boolean) is
-- unchanged and no caller (edge function / SQL) reads the result columns.
--
-- This migration also:
--   * replays every row in commission_failures through the fixed function and
--     deletes each row once replayed (idempotent on re-run),
--   * re-syncs referral_stats.total_earnings from the commissions ledger.

-- ============================================================
-- 1. Allow the 'indirect' commission type (idempotent, keeps the
--    hardened Aug-13/00 migration self-contained)
-- ============================================================
...
-- ============================================================
-- 2. Rewrite record_ambassador_commission with non-shadowing OUT params
-- ============================================================
...
-- ============================================================
-- 3. Replay failed commission attempts (idempotent)
-- ============================================================
DO $$
DECLARE
  f RECORD;
BEGIN
  FOR f IN SELECT id, user_id, amount FROM public.commission_failures LOOP
    BEGIN
      PERFORM public.record_ambassador_commission(f.user_id, f.amount, FALSE);
      DELETE FROM public.commission_failures WHERE id = f.id;
      RAISE NOTICE 'Replayed commission for failure %', f.id;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Replay still failing for %: %', f.id, SQLERRM;
    END;
  END LOOP;
END $$;
